...
{	
	...
	m_pRecordset->CursorType = adOpenStatic;
	if(bIsSelect || nOption == openQuery || nOption == openUnknown || nOption == openUpdate)
	{
		if(nOption == openUpdate)
		{
			m_pRecordset->CursorType = adOpenKeyset;
			hr = m_pRecordset->Open((LPCTSTR)m_strQuery, _variant_t((IDispatch*)mpdb, TRUE), 
...
BOOL CADORecordset::GetFieldValue(LPCTSTR lpFieldName, CString& strValue, CString strDateFormat)
{
	...
	vtFld = pField->Value;
		...
		case VT_DATE:
			...
			{
				_bstr_t	bstr(vtFld);
				str = (LPCTSTR) bstr;	// use operator const wchar_t*( )
			}
#endif
			break;
		case VT_EMPTY:
...
BOOL CADODatabase::Execute(LPCTSTR lpstrExec)
{
	...
	hr = m_pConnection->Execute(_bstr_t(lpstrExec), &vRecords, adCmdText);
	m_nRecordsAffected = vRecords.iVal;
...
BOOL CADORecordset::PutFieldValue(_variant_t vtIndex, _variant_t vtFld)
{
	...
	pField = m_pRecordset->Fields->GetItem(vtIndex);
