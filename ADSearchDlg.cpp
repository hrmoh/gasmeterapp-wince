// ADSearchDlg.cpp : implementation file
//
...
CADSearchDlg::CADSearchDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CADSearchDlg::IDD, pParent)
{
	//{{AFX_DATA_INIT(CADSearchDlg)
	m_sSearch = _T("");
	//}}AFX_DATA_INIT
}


void CADSearchDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	//{{AFX_DATA_MAP(CADSearchDlg)
	DDX_Text(pDX, IDC_ESEARCH, m_sSearch);
	//}}AFX_DATA_MAP
}


BEGIN_MESSAGE_MAP(CADSearchDlg, CDialog)
	//{{AFX_MSG_MAP(CADSearchDlg)
...
/////////////////////////////////////////////////////////////////////////////
// CADSearchDlg message handlers

BOOL CADSearchDlg::OnInitDialog() 
{
...
void CADSearchDlg::PersianizeMe()
{
	CString sWindowText;
...
void CADSearchDlg::FullScreenMe()
{
	CRect rect;
...
void CADSearchDlg::OnSearch() 
{
	UpdateData();	

	if(m_sSearch.GetLength()<5) return;
	
	CADORecordset rs=CADORecordset(&g_db);
	BOOL bOk=rs.Open(_T("SELECT * FROM WhiteList WHERE (WhiteList.AddressCode LIKE '%"+m_sSearch+"%');"), CADORecordset::openQuery);
	if(bOk)
	{
		if(rs.IsEof())
		{
...
void CADSearchDlg::OnChangemetheod() 
{
	CSearchMethodSelDlg	dlg;
	dlg.m_iSelectedMethod=3;
	dlg.DoModal();
	if(dlg.m_iSelectedMethod!=3)
	{
